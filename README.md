...
Before running `setup.sh`, install the required packages:
...
> If you use **Azeroth Eras Control**, the application can perform this prerequisite installation for you on the remote server before uploading and launching `setup.sh`.

Then, to start the installation, simply run:

```bash
./setup.sh
```

I made a personal script to install AzerothCore and compile it when finished. This custom script automates the setup and deployment of an [**AzerothCore**](https://github.com/azerothcore/azerothcore-wotlk) server with the custom modules I use on a fresh Debian VM (Debian 13.6.0 as of 09/08/26).
...
* **System prerequisites**:
  * The required Debian packages are listed in the Prerequisites section above.
  * They can also be installed automatically from Azeroth Eras Control.
...
* **Compilation & Manual Post-Setup Steps:**
  * The script handles AzerothCore dependency installation (`./acore.sh install-deps`) and then offers to launch the full compilation process (you **must** press **y** when prompted if you want compilation to start immediately).
